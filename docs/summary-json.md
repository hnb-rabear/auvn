...
- `bottomHunter`: kết quả Bottom Hunter hiện tại, lớp NGỮ CẢNH nằm ngoài trục mua.
  ...
  - `crashMode`: `true` khi Bear DCA ở pha `acute` **HOẶC** giá sụt ≥8% so với đỉnh 42 phiên (nới 2026-09-25, `isCrashDisplayMode` — cổng cũ chỉ xét độ sâu so với đỉnh mọi thời đại nên bỏ sót 37 ngày prob≥55% đúng 0/37). Khi đó `prob` có trọng số recency lạc quan giả, consumer phải đọc `probUnweighted` — đúng cổng hiển thị mà web dùng. Lưu ý: `probUnweighted` trong chế độ này cũng thường ≥55%, nên hãy coi cờ là "độ tin cậy thấp", không phải số đã sửa đúng.
