...
- Tần suất dùng xem-quá-khứ: **thường xuyên** → bộ chọn thời gian luôn hiện, tham khảo Máy Thời Gian. Vẫn 3 tầng như cũ (thô/tinh/chính-xác) nhưng tầng thô đổi từ slider-mù-ngữ-cảnh thành sparkline giá tự-giải-thích → bỏ được dòng chú thích "kéo = tháng · ◀▶ = ngày · hoặc chọn:".
...
- **Sparkline giá toàn lịch sử** (SVG `path` từ `timeline.points[*].price`, giống pattern Máy Thời Gian nhưng KHÔNG zoom/pan), cao ~64px, `preserveAspectRatio="none"`, vạch cursor dọc tại ngày đang chọn. **`touch-action: pan-y`** (KHÔNG copy `touch-action: none` của `.tm-chart` — sparkline thấp nằm giữa trang dài, phải cho vuốt dọc cuộn trang xuyên qua, chỉ bắt kéo ngang).
...
- **2 nút ◀▶** dạng FAB nổi 2 mép biểu đồ (kiểu `.tm-fab` của Máy Thời Gian) = nhích từng phiên (disable ở 2 đầu). `.tm-fab` là `position:absolute` nên cần wrapper `position:relative` riêng (`.bdo-sparkwrap`); FAB 36px trên sparkline 64px hơi to — được phép làm biến thể nhỏ hơn (`.bdo-fab`) thay vì sửa `.tm-fab`.
...
- CSS class mới tiền tố `bdo-` (vd `.bdo-spark`, `.bdo-sparkwrap`, `.bdo-blocks`, `.bdo-block`, `.bdo-cell`); **KHÔNG đụng `.bt-table`** (bảng backtest trong `Dashboard.tsx` đang dùng chung — nơi duy nhất còn lại sau khi card này bỏ bảng) và không đụng `.tm-*` của TimeMachine (chỉ mô phỏng pattern, có thể tái dùng class `.tm-fab`/`.tm-daterange` nếu khớp nguyên trạng — không sửa chúng).
