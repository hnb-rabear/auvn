...
**Thay đổi:** `prob` của cả 2 tầng (live + `bottomHistory` walk-forward, cùng công thức để giữ bất biến "điểm cuối == gauge") = base-rate cùng bin có trọng số `0.5^(tuổi/504)` (`BOTTOM_CONFIG.recencyHalflife`). CI tính **cùng scheme trọng số** (bài học reliability patch Bear Downside: estimate phải ∈ CI của chính nó, khoá bằng test). Giữ `probUnweighted` (số cũ) + `ess` (cỡ mẫu hiệu dụng, hôm nay ~190/tầng).

**Sửa CI 2026-09-14 — bootstrap theo CỤM thay vì theo mảng đã lọc.** `weightedBlockBootstrapCi` nhận `favArr` — mảng CHỈ gồm các ngày cùng bin — nên khoảng cách lịch giữa chúng biến mất: hai ngày cùng bin cách nhau nhiều năm bị coi là liền kề, còn một chùm quá bán vài tuần bị đếm thành ngần ấy quan sát độc lập ⇒ CI hẹp giả. Nay dùng `clusterBootstrapCiWeighted` với vị trí thật `r.i` của từng quan sát và khối = H phiên (`src/lib/bottom.ts`, cả gauge live lẫn mọi nút `bottomHistory`). Kết quả: cycle 33,2–66,6% → **38,8–65,8%**, swing 39,2–62,8% → **39,4–63,1%** (prob không đổi: 54,8 / 51,7). Chi tiết phương pháp + cảnh báo "đừng gộp-theo-gap vì hiệu ứng dây chuyền" ghi trong CLAUDE.md và `docs/presets.md` Giới hạn #1; khóa bằng `tests/calendar-bootstrap.test.ts`.
